...

{
  ...
  for (unsigned ph = 0; ph < _num_phases; ++ph)
    ...
    {
       r += _WI * _rho_well[_qp] / _mu_well[_qp] * (_pp[_qp][ph] - _p_well[_qp]);
    }
...
Real
PorousFlowPeacemanBoreholeMass::computeQpJac(unsigned int pvar)
{
Real j = 0.0;

for (unsigned ph = 0; ph < _num_phases; ++ph)
  {
  j += _WI * _rho_well[_qp] / _mu_well[_qp] * _dpp_dvar[_qp][ph][pvar] * _phi[_j][_qp];
  }
