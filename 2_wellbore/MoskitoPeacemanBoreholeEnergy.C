...
MoskitoPeacemanBoreholeEnergy::computeQpResidual()
{
  ...
  r -=  2.0 * PI * _rto[_qp] * _Uto[_qp] * (_T_res[_qp] * _scale_T * _postprocessor_T * _function_T.value(_t, _q_point[_qp]) - _u[_qp]) / _area[_qp];
...
MoskitoPeacemanBoreholeEnergy::computeQpJacobian()
{
  ...
  j +=  2.0 * PI * _rto[_qp] * _Uto[_qp]  * _phi[_j][_qp] / _area[_qp];
