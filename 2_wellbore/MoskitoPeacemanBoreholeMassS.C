...
#include "MoskitoPeacemanBoreholeMassS.h"
#include "Function.h"

registerMooseObject("MoskitoApp", MoskitoPeacemanBoreholeMassS);

InputParameters
MoskitoPeacemanBoreholeMassS::validParams()
{
...
MoskitoPeacemanBoreholeMassS::MoskitoPeacemanBoreholeMassS(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
MoskitoPeacemanBoreholeMassS::computeQpResidual()
{

  return _WI * _u[_qp] / _mu[_qp] * (_p[_qp] - _p_res[_qp] * _scale * _postprocessor * _function.value(_t, _q_point[_qp])) * _test[_i][_qp];

}

Real
MoskitoPeacemanBoreholeMassS::computeQpJacobian()
{
  Real j = 0.0;

  j += _phi[_j][_qp] / _mu[_qp] * (_p[_qp] - _p_res[_qp] * _scale * _postprocessor * _function.value(_t, _q_point[_qp]));

  return _WI * j * _test[_i][_qp];
}

Real
MoskitoPeacemanBoreholeMassS::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real j = 0.0;
  ...
  {
    j += -_dmu_dT[_qp] * _phi[_j][_qp] / (_mu[_qp] * _mu[_qp]);
    j *= (_p[_qp] - _p_res[_qp] * _scale * _postprocessor * _function.value(_t, _q_point[_qp]));
  }


  return _WI * _u[_qp] * j * _test[_i][_qp];
}
